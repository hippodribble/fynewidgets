...
func (d *Datum) FitDevice(size fyne.Size) error {
	...
	level := -int(math.Log2(float64(scale)) + .5)                               // convert scale to level
	level = min(max(level, 0), d.Pyramid.Height()-1)                            // constrain level to what is available in the pyramid

	d.Pyramid.level = level
	d.Scale = scale
	d.DeviceCoords = &mid
	d.ImageCoords = &MID
	d.Ticks = ticks

	// log.Println("Datum modified:", d)
...
func (d *Datum) GetCurrentImage(size fyne.Size) (*image.NRGBA, int,error) {
	// log.Println("GetCurrentImage called")
	...
	// log.Println("Rectangles", rSource, rDest)
...
func (d *Datum) ChangeProjection(p fyne.Position, scalerequested float32) error {
	// log.Println("ChangeProjection called")
	ip, err := d.TransformDeviceToFullImage(p)
	if err != nil {
		return errors.Wrap(err, "TransformDeviceToFullImage, in Datum.ChangeProjection")
	}

	// log.Printf("OLD: Ticks=%d Scale=%5.2f Level:=%d - requesting scale=%5.2f\n", d.Ticks, d.Scale, d.Pyramid.level, scalerequested)
	ticks := fynewidgets.FloatScaleToTicks(scalerequested, d.Sensitivity) // convert to discrete ticks
	// log.Println("Requested ticks", ticks)
	newscale := fynewidgets.TickScaleToFloatScale(ticks, d.Sensitivity) // convert back to float
	// log.Println("corresponding to scale", newscale)
	level := -int(math.Log2(float64(newscale)) - .5) // convert scale to level
	// log.Println("at level", level)
	level = min(max(level, 0), d.Pyramid.Height()-1) // constrain level to what is available in the pyramid
	// log.Println("constrained to level", level)
	...
	d.Pyramid.level = level // update level

	// log.Printf("NEW: Ticks=%d Scale=%5.2f Level:=%d\n", d.Ticks, d.Scale, d.Pyramid.level)
	return nil
}

func (d *Datum) ScaleByTick(p fyne.Position, delta float32) error {
	// log.Println("ScaleByTick called")
	dir := 1
	if delta < 0 {
		dir = -1
	}
	...
	// log.Println("Requesting scale change to ", newscale, "based on new ticks", newticks)
